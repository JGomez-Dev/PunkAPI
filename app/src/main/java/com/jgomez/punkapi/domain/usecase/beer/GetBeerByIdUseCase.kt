...
class GetBeerByIdUseCase @Inject constructor(private val beerRepository: BeerRepository) {
    operator fun invoke(id: Int): Flow<Either<BeerModel, GenericError>> = flow {
        emit(beerRepository.getBeerById(id))
    }
}